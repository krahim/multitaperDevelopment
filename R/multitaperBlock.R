...
## ###########################################################################################3
## ### added block with out adaptive weights, goal is to merge this
## ## at some point
## it seems mtm$sas is what I want to average...
...
## ## do we need this,,,, looks like not...
## spec.mtm.block.noAdaptiveWts <- function(timeSeries,
##                                           k,
##                                           nw,
##                                           nFFT,
##                                           nBlockLen = length(timeSeries),
...
##         nPadLen <- nFFT - nBlockLen
##         paddedTaperedData <- rbind(taperedData, matrix(0, nPadLen, k))
##         ## cft are the eigen coefficients
##         cft <- mvfft(paddedTaperedData)
##         cft <- cft[(1+offSet):(nFreqs+offSet),]
##         ## sa is mEigenSpectra
##         ## see  PWLISPR::multitaperSpectralEstimate
##         sa <- abs(cft)**2

##         avgSDF <- apply(sa, 1, mean)
##         ##adaptive <-  NULL
##         ##jk <- NULL
